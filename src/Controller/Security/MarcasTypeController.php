...
use Palopoli\PaloSystem\Form\MarcasTypeForm;
use Symfony\Component\HttpFoundation\Request;
use Doctrine\DBAL\Exception\UniqueConstraintViolationException;
use Symfony\Component\HttpFoundation\File\UploadedFile;
use Symfony\Component\Filesystem\Filesystem;

class MarcasTypeController extends ContainerAware
{
    /**
     * Lista.
     *
    ...
    {
        $marcas_type = $this->db()->fetchAll('SELECT * FROM `marcas_type`');
    ...
    public function createAction(Request $request)
    {
        ...
        $form = $this->createForm(new MarcasTypeForm(), $marcas_type, array(
            'action' => $this->get('url_generator')->generate('s_marcas_type_create'),
            'method' => 'POST',
        ));
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                if ($data['image'] instanceof UploadedFile) {
                    $image = $data['image'];
                    $fs = new Filesystem();
                    $directory = web_path('upload/marcas');
                ...
                try {
                    $insert_query = 'INSERT INTO `marcas_type` (`title`, `image`, `enabled`, `created_at`, `updated_at`) VALUES ( ?, ?, ?, NOW(), NOW())';
                    ...
                    return $this->redirect('s_marcas_type');
    ...
    {
        $marcas_type = $this->get('db')->fetchAssoc('SELECT * FROM `marcas_type` WHERE `id` = ? LIMIT 1;', array($id));

        if ($marcas_type === false) {
            $this->flashMessage()->add('warning', array('message' => 'Desculpe, mais a pagina não foi encontrada.'));

            return $this->redirect('s_marcas_type');
        }

        $image_name = $marcas_type['image'];
        unset($marcas_type['image']);

        $form = $this->createForm(new MarcasTypeForm(), $marcas_type, array(
            'action' => $this->get('url_generator')->generate('s_marcas_type_edit', array('id' => $id)),
            'method' => 'POST',
        ));
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                try {
                    $update_query = 'UPDATE `marcas_type` SET `title` = ?, `image` = ?,  `enabled` = ?, `updated_at` = NOW() WHERE `id` = ? LIMIT 1';
    ...
    public function deleteAction(Request $request)
    {
        ...
        $row_sql = $this->get('db')->fetchAssoc('SELECT * FROM `marcas_type` WHERE `id` = ? LIMIT 1;', array($id));

        if ($row_sql === false) {
            $this->flashMessage()->add('warning', array('message' => 'Desculpe, mais não foi encontrado.'));
        } else {
            $this->get('db')->executeUpdate('DELETE FROM `marcas_type` WHERE `id` = ?', array($id));
