...
use Palopoli\PaloSystem\Form\ImpulsosTypeForm;
use Symfony\Component\HttpFoundation\Request;
use Doctrine\DBAL\Exception\UniqueConstraintViolationException;
use Symfony\Component\HttpFoundation\File\UploadedFile;
use Symfony\Component\Filesystem\Filesystem;

class ImpulsosTypeController extends ContainerAware
{
    /**
     * Lista.
     *
    ...
    {
        $impulsos_type = $this->db()->fetchAll('SELECT * FROM `impulsos_type`');
    ...
    public function createAction(Request $request)
    {
        ...
        $form = $this->createForm(new ImpulsosTypeForm(), $impulsos_type, array(
            'action' => $this->get('url_generator')->generate('s_impulsos_type_create'),
            'method' => 'POST',
        ));
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                if ($data['image'] instanceof UploadedFile) {
                    $image = $data['image'];
                    $fs = new Filesystem();
                    $directory = web_path('upload/impulsos');
                ...
                try {
                    $insert_query = 'INSERT INTO `impulsos_type` (`title`, `image`, `enabled`, `created_at`, `updated_at`) VALUES ( ?, ?, ?, NOW(), NOW())';
                    ...
                    return $this->redirect('s_impulsos_type');
    ...
    {
        $impulsos_type = $this->get('db')->fetchAssoc('SELECT * FROM `impulsos_type` WHERE `id` = ? LIMIT 1;', array($id));

        if ($impulsos_type === false) {
            $this->flashMessage()->add('warning', array('message' => 'Desculpe, mais a pagina não foi encontrada.'));

            return $this->redirect('s_impulsos_type');
        }

        $image_name = $impulsos_type['image'];
        unset($impulsos_type['image']);

        $form = $this->createForm(new ImpulsosTypeForm(), $impulsos_type, array(
            'action' => $this->get('url_generator')->generate('s_impulsos_type_edit', array('id' => $id)),
            'method' => 'POST',
        ));
        ...
        if ('POST' == $request->getMethod()) {
            ...
            if ($form->isValid()) {
                ...
                try {
                    $update_query = 'UPDATE `impulsos_type` SET `title` = ?, `image` = ?,  `enabled` = ?, `updated_at` = NOW() WHERE `id` = ? LIMIT 1';
    ...
    public function deleteAction(Request $request)
    {
        ...
        $row_sql = $this->get('db')->fetchAssoc('SELECT * FROM `impulsos_type` WHERE `id` = ? LIMIT 1;', array($id));

        if ($row_sql === false) {
            $this->flashMessage()->add('warning', array('message' => 'Desculpe, mais não foi encontrado.'));
        } else {
            $this->get('db')->executeUpdate('DELETE FROM `impulsos_type` WHERE `id` = ?', array($id));
